...
- Notes:
  ...
  - Go source mode is selected when the chosen entrypoint ends with `.go`; `kinnoo run --preflight` checks Go toolchain/runtime constraints.
  - Go binary mode is selected when the chosen entrypoint does not end with `.go`; `kinnoo run --preflight` skips Go toolchain checks and runs binary compatibility checks.
  - In Go binary mode, preflight can hard-fail on wrong host OS/architecture, unsupported executable format, or non-executable artifacts.
...
- Notes:
  ...
  - Go framework + language combinations supported by init: `gemini`, `chatgpt`, `claude-chat`, `mcp-client`, `mcp-server`, `no-framework` with `--language go`.

### Go preflight status semantics

- `kinnoo run --preflight` prints per-check labels: `- [PASS]`, `- [WARN]`, `- [FAIL]`.
- Final status lines are deterministic:
  - success: `Ready to run` and `Preflight result: PASS`
  - failure: `Not ready to run`, `Remediation summary:`, and `Preflight result: FAIL`
- Go remediation guidance aligns with runtime diagnostics:
  - missing toolchain in source mode: install Go or configure `runtime.path`
  - wrong host compatibility: rebuild with host `GOOS` and `GOARCH`
  - unsupported format: rebuild to supported executable format (Mach-O/ELF/PE)
  - non-executable binary: apply execute permission (`chmod +x <entrypoint>`)
